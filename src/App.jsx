  ...
  return (
    <header className="site-header">
      <Link className="brand" to="/" aria-label="Livo Arts & Designs home">
        <img src="/logo.jpg" alt="Livo Arts & Designs" className="brand-logo" />
      </Link>
...
function FloatingActions() {
  return (
    <div className="floating-actions" aria-label="Quick contact links">
      ...
      <a href={emailUrl} aria-label="Send email">
        <MdEmail />
      </a>
    </div>
  )
  ...
  return (
    <motion.main variants={pageVariants} initial="initial" animate="animate" exit="exit">
      <section className="hero-section">
        ...
        >
          ...
          <p className="hero-copy">
            Premium residential interiors shaped with rich materials, calm details, and sacred spaces designed for modern Indian homes.
          </p>
      ...
      <section className="stats-band">
        {['Bespoke Design', 'Premium Materials', 'Mandir Specialists', 'Turnkey Vision'].map((item) => (
      ...
      <SectionIntro
        ...
        title="Interiors that feel intimate, cinematic, and enduring."
        text="Every project balances refined planning, handcrafted details, and a warm black-gold palette for an unmistakably premium mood."
      />

      <div className="home-grid">
        {interiors.slice(0, 3).map((item, index) => (
          <ProjectCard key={item} title={item} type="Interior" index={index} />
        ))}
      </div>
    </motion.main>
  )
...
function Portfolio() {
  return (
    <PageShell
      eyebrow="Portfolio"
      title="Luxury project cards and curated interior gallery."
      text="Explore elegant living rooms, suites, foyers, dining spaces, and statement corners designed for premium lifestyles."
    >
      <div className="card-grid">
        {interiors.map((item, index) => (
          <ProjectCard key={item} title={item} type="Interior Gallery" index={index} />
        ))}
      </div>
    </PageShell>
  )
...
function MandirDesigns() {
  return (
    <PageShell
      eyebrow="Mandir designs"
      title="Sacred spaces with rich detailing and peaceful proportion."
      text="From compact home mandirs to grand carved sanctums, each design blends devotion, lighting, material craft, and modern utility."
    >
      <div className="card-grid">
        {mandirs.map((item, index) => (
          <ProjectCard key={item} title={item} type="Mandir Gallery" index={index} />
        ))}
      </div>
    </PageShell>
  )
...
function About() {
  return (
    <PageShell
      eyebrow="About Livo"
      title="A design studio for refined homes and soulful mandirs."
      text="Livo Arts & Designs creates immersive interiors where luxury is quiet, personal, and deeply functional."
    >
      <div className="about-panel">
        <div>
          <h3>Our approach</h3>
          <p>
            We combine mood-led concepts, precise space planning, material storytelling, and detailed execution guidance to make every room feel complete.
          </p>
        </div>
        <ul>
...
function Contact() {
  return (
    <PageShell
      eyebrow="Contact"
      title="Book a luxury design consultation."
      text="Tell us about your home, mandir, or renovation vision. We will help shape it into a premium design experience."
    >
